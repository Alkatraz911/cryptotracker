...
export default function OrbiterImport({ onAdd, onClose }: Props) {
  ...
  async function load() {
    ...
    try {
      ...
      if (!hops.length) { setMsg(diag ?? tr("Переводов не найдено.")); return; }
      setHops(hops);
    } catch (e: any) {
      setMsg(e.message ?? tr("Ошибка запроса к мосту"));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="orbiter-import">
      <p className="muted">
        {tr("Кроссчейн-переводы через мост за выбранный период. Orbiter: прыжок по времени достаёт и старую историю. deBridge (DLN): хранит всю историю, тянется от свежих к старым с адресами отправителя/получателя.")}
      </p>
      ...
      <div className="ltrow">
        <label>{tr("Период")}</label>
        <input type="date" value={from} max={to} onChange={(e) => setFrom(e.target.value)} />
        <span className="muted">—</span>
        <input type="date" value={to} min={from} onChange={(e) => setTo(e.target.value)} />
      </div>
      <div className="ltchecks">
        <label className="lim">{tr("мин $")} <input type="number" min={0} value={minUsd}
          onChange={(e) => setMinUsd(Math.max(0, Number(e.target.value) || 0))} /></label>
        <label className="lim">{tr("лимит")} <input type="number" min={1} max={500} value={limit}
      ...
      <button className="primary" onClick={load} disabled={busy}>
        {busy ? tr("Загрузка…") : tr("Загрузить переводы")}
      </button>

      {hops && hops.length > 0 && (
        <>
          <div className="flash">
            {tr("Найдено")} {hops.length} переводов · сумма ≈ ${totalUsd.toLocaleString("en-US", { maximumFractionDigits: 0 })}
          </div>
          <ul className="orbiter-preview">
            ...
            {hops.length > 12 && <li className="muted">{tr("…ещё")} {hops.length - 12}</li>}
          </ul>
          <button className="primary" onClick={addAll}>{tr("Добавить")} {hops.length} {tr("в граф")}</button>
        </>
      )}
    </div>
  );
